              ...
              <div className="px-6 py-4 border-t border-slate-200 bg-slate-50 flex flex-col sm:flex-row items-center justify-between gap-4 text-sm text-slate-600">
                ...
                <div className="flex gap-2">
                  ...
                  {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                    // Logic to show ranges like 1 2 3 4 5 ... 11
                    // For simplicity in this edit, showing first 5 pages always
                    return i + 1
                  }).map(page => (
                    <button
                      key={page}
                      onClick={() => setCurrentPage(page)}
                      className={`px-3 py-1 rounded border ${currentPage === page ? 'bg-[#0e4a78] text-white border-[#0e4a78]' : 'bg-white border-slate-300 text-slate-700 hover:bg-slate-50'}`}
                    >
                      {page}
                    </button>
                  ))}
                  ...
                  {totalPages > 5 && (
                    <button
                      ...
                      className={`px-3 py-1 rounded border ${currentPage === totalPages ? 'bg-[#0e4a78] text-white border-[#0e4a78]' : 'bg-white border-slate-300 text-slate-700 hover:bg-slate-50'}`}
                    >
                      {totalPages}
                    </button>
                  )}
