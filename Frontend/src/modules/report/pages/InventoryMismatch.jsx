...
const InventoryMismatch = () => {
  ...
  useEffect(() => { setHasQueried(true); fetchMismatch({ from_date: fromDefault(), to_date: todayLocalDT() }) }, []) // eslint-disable-line
  ...
  const handleClear = () => {
    ...
    setSearch('')
    setHasQueried(true)
    fetchMismatch({ from_date: fromDefault(), to_date: todayLocalDT() })
  }
  ...
  const handleExport = () => {
    ...
    XLSX.writeFile(wb, `MismatchContainer_${todayLocalDT().slice(0, 10)}.xlsx`)
  }
