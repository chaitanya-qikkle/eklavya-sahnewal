...
import { FiSearch, FiRefreshCw, FiX, FiChevronDown, FiCalendar, FiHardDrive } from 'react-icons/fi'
import * as XLSX from 'xlsx'
import Navbar from '../../../components/layout/Navbar'
import Footer from '../../../components/layout/Footer'
import { useGetEquipmentQuery, useLazyGetDeviceTransactionSummaryQuery } from '../../../store/api/ymsApi'
...
const DeviceTransactionSummary = () => {
  ...
  const handleExport = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'DeviceTransactionSummary')
    XLSX.writeFile(wb, `DeviceTransactionSummary_${todayLocalDT().slice(0, 10)}.xlsx`)
  }
        ...
        <main className="flex-1 px-4 sm:px-6 lg:px-8 py-8">
          <div className="w-full space-y-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-[#0e4a78]">Device Transaction Summary</h1>
                <p className="text-slate-500 text-sm">Lock-key (LK) equipment transactions by container or device</p>
              </div>
            </div>
              ...
              <div className="p-6 rounded-b-2xl">
                <div className="flex flex-col lg:flex-row lg:items-end gap-4">
                  ...
                  <div className="flex flex-col gap-1.5 relative w-full lg:w-64" ref={eqpBoxRef}>
                    <label className="text-xs font-bold text-slate-600 uppercase tracking-[0.12em]">Equipment</label>
                    <button
                      type="button"
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {isFetching ? <FiRefreshCw className="animate-spin text-base" /> : <FiSearch className="text-base" />}
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden">
              <div className="bg-gradient-to-r from-[#0e4a78] to-[#0a3b61] px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  <h2 className="text-white font-bold text-lg tracking-wide uppercase">Transaction Summary</h2>
                ...
                {isError ? (
                  ...
                  <div className="px-8 py-14 text-center">
                    <div className="w-12 h-12 mx-auto mb-3 rounded-full bg-slate-100 flex items-center justify-center">
                      <FiHardDrive className="text-slate-400 text-xl" />
                    </div>
                      ...
                      {paginatedData.map((row, index) => (
                        <tr key={index} className={`transition-colors ${index % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'} hover:bg-blue-50/50`}>
                          {COLUMNS.map((col) => {
                            ...
                            return (
                              <td key={col.key} className={`px-4 py-3 whitespace-nowrap ${col.key === 'ContNo' ? 'text-slate-800 font-semibold' : 'text-slate-600'}`}>
                                {display}
                              </td>
                            )
                          })}
