...
import { FiCalendar, FiRefreshCw, FiSearch, FiX, FiGitPullRequest } from 'react-icons/fi'
...
const COLUMNS = [
  ...
  { key: 'Proposed_Loc',     label: 'Proposed Location' },
  { key: 'Actual_Loc',       label: 'Actual Location' },
  { key: 'current_Loc',      label: 'Current Location' },
  { key: 'JobCreation',      label: 'Job Creation', format: fmtDate },
  { key: 'JobCompletionDate',label: 'Job Completion', format: fmtDate },
  { key: 'TAT',              label: 'TAT' },
]
...
const ActualVsProposedPlan = () => {
  ...
  const handleSearch = () => {
    setHasQueried(true)
    fetchData({ from_date: fromDate, to_date: toDate })
  }
  ...
  const matchStats = useMemo(() => {
    ...
    const matched = rows.filter((r) => r.Proposed_Loc && r.Actual_Loc && r.Proposed_Loc === r.Actual_Loc).length
          ...
          <div className="w-full space-y-6">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-[#0e4a78] flex items-center justify-center shadow">
                  <FiGitPullRequest className="text-white text-xl" />
                </div>
                <div>
                  <h1 className="text-2xl font-bold text-[#0e4a78]">Actual vs Proposed Plan</h1>
                  <p className="text-slate-500 text-sm">Completed jobs — proposed vs actual container location</p>
                </div>
              </div>

              {matchStats.total > 0 && (
                <div className="flex gap-3">
                  <div className="bg-white rounded-lg shadow border border-slate-200 px-4 py-2.5">
                    <div className="text-[10px] font-bold text-[#0e4a78] uppercase tracking-widest">Total</div>
                    <div className="text-xl font-black text-slate-800">{matchStats.total}</div>
                  </div>
                  <div className="bg-white rounded-lg shadow border border-slate-200 px-4 py-2.5">
                    <div className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest">Matched</div>
                    <div className="text-xl font-black text-emerald-700">{matchStats.matched}</div>
                  </div>
                  <div className="bg-white rounded-lg shadow border border-slate-200 px-4 py-2.5">
                    <div className="text-[10px] font-bold text-amber-600 uppercase tracking-widest">Mismatched</div>
                    <div className="text-xl font-black text-amber-700">{matchStats.mismatched}</div>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden">
              ...
              <div className="p-6">
                ...
                <p className="text-xs text-slate-400 mt-3">Only jobs completed within the selected date range are shown.</p>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden">
              <div className="bg-gradient-to-r from-[#0e4a78] to-[#0a3b61] px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  <h2 className="text-white font-bold text-lg tracking-wide uppercase">Plan vs Actual Detail</h2>
              ...
              <div className="overflow-x-auto">
                {isError ? (
                  ...
                  <div className="px-8 py-12 flex flex-col items-center gap-3 text-slate-400">
                    ...
                    <p className="text-sm font-medium">Loading plan comparison…</p>
                  </div>
                ) : !hasQueried ? (
                  <div className="px-8 py-14 text-center">
                    <div className="w-12 h-12 mx-auto mb-3 rounded-full bg-slate-100 flex items-center justify-center">
                      <FiGitPullRequest className="text-slate-400 text-xl" />
                    </div>
                  ...
                  <table className="min-w-full text-sm">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {filteredData.map((row, index) => {
                        const isMismatch = row.Proposed_Loc && row.Actual_Loc && row.Proposed_Loc !== row.Actual_Loc
                        return (
