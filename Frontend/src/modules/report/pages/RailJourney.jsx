...
import { FiCalendar, FiRefreshCw, FiSearch, FiX, FiActivity } from 'react-icons/fi'
...
const RailJourney = () => {
  ...
  useEffect(() => { setHasQueried(true); fetchRailJourney({ from_date: fromDefault(), to_date: todayLocalDT() }) }, []) // eslint-disable-line

  const rows = Array.isArray(data?.data) ? data.data : []

  const handleSearch = () => {
    setHasQueried(true)
    fetchRailJourney({ from_date: fromDate, to_date: toDate })
  }
  ...
  const handleClear = () => {
    ...
    setSearch('')
    setHasQueried(true)
    fetchRailJourney({ from_date: fromDefault(), to_date: todayLocalDT() })
  }
  ...
  const handleExport = () => {
    ...
    XLSX.writeFile(wb, `RailJourney_${todayLocalDT().slice(0, 10)}.xlsx`)
  }
        ...
        <main className="flex-1 px-4 sm:px-6 lg:px-8 py-8">
          <div className="w-full space-y-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-[#0e4a78]">Rail Journey</h1>
                <p className="text-slate-500 text-sm">Document-wise rail release and gate-in progress</p>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden">
              <div className="bg-gradient-to-r from-[#0e4a78] to-[#0a3b61] px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  <h2 className="text-white font-bold text-lg tracking-wide uppercase">Rail Journey Report</h2>
              ...
              <div className="overflow-x-auto">
                {isError ? (
                  ...
                  <div className="px-8 py-12 flex flex-col items-center gap-3 text-slate-400">
                    ...
                    <p className="text-sm font-medium">Loading rail journey data…</p>
                  </div>
                ) : !hasQueried ? (
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {filteredData.map((row, index) => (
                        <tr
                          ...
                          className={`transition-colors cursor-pointer ${index % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'} hover:bg-blue-50/50`}
                        >
                          {COLUMNS.map((col) => {
                            ...
                            return (
                              <td
                                ...
                                className={`px-4 py-3 whitespace-nowrap ${
                                  col.key === 'DocumentNo' ? 'text-[#0e4a78] font-semibold underline-offset-2 hover:underline' : 'text-slate-600'
                                }`}
                              >
                                {display}
                              </td>
