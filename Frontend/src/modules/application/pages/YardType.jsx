          ...
          <div className="w-full space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <section className="bg-white/95 rounded-2xl shadow-xl border border-slate-300 overflow-hidden flex flex-col">
                ...
                <div className="px-6 py-3 border-t border-slate-200 bg-slate-50 flex flex-col sm:flex-row items-center justify-between gap-3 text-sm text-slate-600">
                  <div>
                    Showing {Math.min((currentPage - 1) * pageSize + 1, filteredYardTypes.length)} to {Math.min(currentPage * pageSize, filteredYardTypes.length)} of {filteredYardTypes.length} entries
                  </div>
                  <div className="flex gap-2">
                    <button
                      onClick={() => setCurrentPage(p => Math.max(1, p - 1))}
                      disabled={currentPage === 1}
                      className="px-3 py-1 rounded border border-slate-300 bg-white hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed text-[#0e4a78] font-semibold text-xs"
                    >
                      Previous
                    </button>
                    <button className="px-3 py-1 rounded border bg-[#0e4a78] text-white border-[#0e4a78] text-xs">
                      {currentPage}
                    </button>
                    <button
