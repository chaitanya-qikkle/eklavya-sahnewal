  ...
  const customerTable = (
    <section className="bg-white/95 rounded-2xl shadow-xl border border-slate-300 overflow-hidden flex flex-col">
      ...
      <div className="p-6 space-y-4 flex-1 flex flex-col">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 text-sm text-slate-600 font-medium pt-4 border-t border-slate-200">
          <div>
            Showing <span className="text-[#0e4a78] font-bold">{Math.min((currentPage - 1) * 10 + 1, filteredCustomers.length)}</span> to{' '}
            <span className="text-[#0e4a78] font-bold">{Math.min(currentPage * 10, filteredCustomers.length)}</span> of{' '}
            <span className="text-[#0e4a78] font-bold">{filteredCustomers.length}</span> entries
          </div>
          {totalPages > 1 && (
