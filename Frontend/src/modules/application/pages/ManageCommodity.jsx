            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <section className="bg-white/95 rounded-2xl shadow-xl border border-slate-300 overflow-hidden flex flex-col">
                ...
                <div className="px-6 py-4 border-t border-slate-200 bg-slate-50 flex flex-col sm:flex-row items-center justify-between gap-3 text-sm text-slate-600 font-medium">
                  <div>
                    Showing <span className="text-[#0e4a78] font-bold">{Math.min((currentPage - 1) * pageSize + 1, filteredCommodities.length)}</span> to <span className="text-[#0e4a78] font-bold">{Math.min(currentPage * pageSize, filteredCommodities.length)}</span> of <span className="text-[#0e4a78] font-bold">{filteredCommodities.length}</span> entries
                  </div>
                  {totalPages > 1 && (
